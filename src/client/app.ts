...
function showRegistrationRequiredOverlay(reason = 'playback') {
    const title = reason === 'smart-shuffle'
        ? t('smart_shuffle_auth_title', 'Register to use Smart Shuffle')
        : t('playback_auth_title', 'Register to play music');
    const message = reason === 'smart-shuffle'
        ? t('smart_shuffle_auth_message', 'Register to use Smart Shuffle')
        : t('playback_auth_message', 'Register to play music');

    if (typeof window.showMusicRestrictionOverlay === 'function') {
        window.showMusicRestrictionOverlay({ title, message, authTab: 'register' });
        return;
    }
...
function shouldBlockGuestHomePlayback() {
    return !isUserAuthenticated() && isHomePlaybackTab();
}

function bindArtistLinkElement(element) {
    if (!element || element.dataset.artistLinkBound === '1') return;
    element.dataset.artistLinkBound = '1';

    element.addEventListener('click', (event) => {
        const artistName = String(element.dataset.artistName || '').trim();
        if (!artistName) return;
        event.preventDefault();
        event.stopPropagation();
        window.openHomeArtist(artistName);
    });
...
function applyArtistLinkState(element, artistName = '') {
    if (!element) return;
    bindArtistLinkElement(element);
    const normalizedName = String(artistName || '').trim();
    element.dataset.artistName = normalizedName;
    if (normalizedName) {
        element.classList.add('artist-link');
        element.setAttribute('role', 'button');
        element.setAttribute('tabindex', '0');
        element.title = `${t('artist', 'Artist')}: ${normalizedName}`;
        return;
    }
    element.classList.remove('artist-link');
    element.removeAttribute('role');
    element.removeAttribute('tabindex');
    element.removeAttribute('title');
}

function initArtistLinkTargets() {
    [
        'artistName',
        'homeMiniArtist',
        'homeAlbumMiniArtist',
        'homeArtistMiniArtist',
        'playlistDetailMiniArtist',
        'mobileFullArtist'
    ].forEach((id) => {
        const element = document.getElementById(id);
        if (!element) return;
        applyArtistLinkState(element, element.textContent || '');
    });
}
...
let homeArtistState = null;
let homeArtistReturnTab = 'home';
...
function rememberLastNonInfoTab(tab) {
    if (!isInfoTab(tab) && isRestorableMainTab(tab)) {
        lastNonInfoTab = tab;
    }
}

function closeInfoTabToLastContext() {
    const targetTab = isRestorableMainTab(lastNonInfoTab) && !isInfoTab(lastNonInfoTab)
        ? lastNonInfoTab
        : 'home';
...
function renderMiniQueueModal() {
    ...
    tracks.forEach((track, idx) => {
        ...
        const queueTrackId = source === 'navidrome'
            ? (track.navidromeId || track.url || track.id)
            : track.id;
        ...
        playNextBtn.innerHTML = '<i data-lucide="list-plus"></i>';
        playNextBtn.onclick = (event) => {
            event.stopPropagation();
            if (source === 'navidrome') {
                window.addToQueueNextNavidrome(String(queueTrackId));
            } else if (window.addToQueueNextLocal) {
                window.addToQueueNextLocal(queueTrackId);
            }
        };

        const addToPlaylistBtn = document.createElement('button');
        addToPlaylistBtn.className = 'mini-btn';
        addToPlaylistBtn.title = t('add_to_playlist', 'Add to playlist');
        addToPlaylistBtn.innerHTML = '<i data-lucide="plus"></i>';
        addToPlaylistBtn.onclick = (event) => {
            event.stopPropagation();
            window.tempPendingTrack = track;
            window.openPlaylistPickerMulti(queueTrackId, source);
        };

        const favBtn = document.createElement('button');
        favBtn.className = 'mini-btn';
        favBtn.title = t('favorite', 'Favorite');
        favBtn.innerHTML = `<i data-lucide="heart" style="fill: ${track.isFavorite ? 'var(--accent)' : 'none'}; color: ${track.isFavorite ? 'var(--accent)' : 'currentColor'}"></i>`;
        favBtn.onclick = (event) => {
            event.stopPropagation();
            Promise.resolve(window.toggleFav(queueTrackId, source))
                .catch(() => {})
                .finally(() => {
                    renderMiniQueueModal();
                    renderSidebarQueue();
        ...
        removeBtn.onclick = (event) => {
            ...
            window.removeFromQueue(queueTrackId);
        };
        ...
        row.appendChild(addToPlaylistBtn);
...
function isBulkSelectionTab(tab = state.currentTab) {
    return tab === 'all' || tab === 'fav' || tab === 'home-album' || tab === 'home-artist' || typeof tab === 'number';
}
...
window.disableBulkSelection = (clear = true) => {
    ...
    if (state.currentTab === 'home-album') {
        renderHomeAlbumTab();
    } else if (state.currentTab === 'home-artist') {
        renderHomeArtistTab();
...
function playHomeTrack(track) {
    if (!track) return;
    if (shouldBlockGuestHomePlayback()) {
        showRegistrationRequiredOverlay('playback');
        return;
    }
...
async function ensureNavidromeSongsLoadedForArtistPage() {
    if (Array.isArray(state.navidromeSongs) && state.navidromeSongs.length) {
        return state.navidromeSongs;
    }
    if (typeof window.getAllNavidromeSongs !== 'function') {
        return [];
    }
    try {
        const songs = await window.getAllNavidromeSongs();
        state.navidromeSongs = Array.isArray(songs) ? songs : [];
    } catch (error) {
        console.warn('[ARTIST] Failed to preload Navidrome songs:', error);
        state.navidromeSongs = [];
    }
    return state.navidromeSongs;
}

function mapArtistSongsFromCache(artistName = '') {
    const artistKey = normalizeArtistKey(artistName);
    if (!artistKey) return [];

    const normalizedSongs = (state.navidromeSongs || [])
        .map(normalizeHomeTrack)
        .filter((track) => track?.source === 'navidrome');

    let matches = normalizedSongs.filter((track) => normalizeArtistKey(track.artist) === artistKey);
    if (!matches.length) {
        matches = normalizedSongs.filter((track) => normalizeArtistKey(track.artist).includes(artistKey));
    }

    const seen = new Set();
    const deduped = [];
    matches.forEach((track) => {
        const key = String(track.navidromeId || track.id || track.url || '').trim();
    ...
    return deduped.sort((a, b) => {
        const albumA = String(a?.album || '').toLowerCase();
        const albumB = String(b?.album || '').toLowerCase();
        if (albumA !== albumB) return albumA.localeCompare(albumB);
        return String(a?.title || '').localeCompare(String(b?.title || ''));
    });
}

function mapArtistAlbums(artistName = '', songs = []) {
    const artistKey = normalizeArtistKey(artistName);
    const byAlbum = new Map();
    ...
    songs.forEach((track) => {
        ...
        const key = `${albumId || albumName.toLowerCase()}`;
        if (!byAlbum.has(key)) {
            byAlbum.set(key, {
                id: albumId || '',
                title: albumName,
        ...
        if (!entry.sampleTrack) entry.sampleTrack = track;
    });

    (mediaAlbumsCatalogCache || []).forEach((album) => {
        const albumArtistKey = normalizeArtistKey(album?.artist || '');
        if (!artistKey || albumArtistKey !== artistKey) return;
        const albumId = String(album?.id || '').trim();
        const albumName = String(album?.title || '').trim();
        if (!albumId && !albumName) return;
        const key = `${albumId || albumName.toLowerCase()}`;
        if (!byAlbum.has(key)) {
            byAlbum.set(key, {
                id: albumId,
                title: albumName || t('unknown_album', 'Unknown album'),
                artist: album?.artist || artistName,
                cover: album?.cover || '',
                songCount: Number(album?.songCount || 0) || 0,
                sampleTrack: null
            });
            return;
        }
        const entry = byAlbum.get(key);
        if (!entry.id && albumId) entry.id = albumId;
        if (!entry.cover && album?.cover) entry.cover = album.cover;
        if (entry.songCount <= 0 && Number(album?.songCount || 0) > 0) {
            entry.songCount = Number(album.songCount);
        }
    });

    return Array.from(byAlbum.values()).sort((a, b) =>
        String(a?.title || '').localeCompare(String(b?.title || ''))
    );
}

function renderHomeArtistTab() {
    const section = document.getElementById('homeArtistSection');
    const titleEl = document.getElementById('homeArtistTitle');
    const subtitleEl = document.getElementById('homeArtistSubtitle');
    const coverEl = document.getElementById('homeArtistCover');
    const albumsGrid = document.getElementById('homeArtistAlbumsGrid');
    const tracksList = document.getElementById('homeArtistList');
    const playBtn = document.getElementById('homeArtistPlayBtn');
    const shuffleBtn = document.getElementById('homeArtistShuffleBtn');
    const addAllBtn = document.getElementById('homeArtistAddAllBtn');
    if (!section || !titleEl || !subtitleEl || !coverEl || !albumsGrid || !tracksList) return;

    if (!homeArtistState) {
        titleEl.textContent = t('unknown_artist', 'Unknown Artist');
        subtitleEl.textContent = t('no_tracks_found', 'No tracks found');
        applyImgFallback(coverEl, DEFAULT_COVER);
        albumsGrid.innerHTML = `<div class="home-empty">${t('home_albums_empty', 'No albums found in Navidrome.')}</div>`;
        tracksList.innerHTML = `<div class="home-empty">${t('no_tracks_found', 'No tracks found')}</div>`;
        state.homeArtistSongs = [];
        if (playBtn) playBtn.disabled = true;
        if (shuffleBtn) shuffleBtn.disabled = true;
        if (addAllBtn) addAllBtn.disabled = true;
        return;
    ...
    const songs = (homeArtistState.songs || []).map(normalizeHomeTrack).filter(Boolean);
    const albums = Array.isArray(homeArtistState.albums) ? homeArtistState.albums : [];
    state.homeArtistSongs = songs;

    titleEl.textContent = homeArtistState.name || t('unknown_artist', 'Unknown Artist');
    subtitleEl.textContent = `${albums.length} ${t('albums', 'Albums')} · ${songs.length} ${t('tracks', 'tracks')}`;
    applyImgFallback(coverEl, homeArtistState.cover || DEFAULT_COVER);
    ...
    if (!albums.length) {
        ...
        albums.forEach((album) => {
            ...
            const albumTitle = album.title || t('unknown_album', 'Unknown album');
            const cover = resolveCover(album.cover || homeArtistState.cover || DEFAULT_COVER);
            const canOpen = !!(album.id || album.sampleTrack);
            card.innerHTML = `
                <img src="${cover}" alt="${albumTitle}" onerror="this.src='${DEFAULT_COVER}'">
                <div class="home-card-title">${albumTitle}</div>
                <div class="home-card-subtitle">${album.artist || homeArtistState.name || t('unknown_artist', 'Unknown Artist')}</div>
            ...
            if (canOpen) {
                card.onclick = async () => {
                    if (album.id) {
                        await window.openHomeAlbumById(album.id, album);
                    } else if (album.sampleTrack) {
                        await window.openHomeAlbumTab(album.sampleTrack);
                    }
                };
            }
    ...
    songs.forEach((track) => {
        ...
        const row = document.createElement('div');
        row.className = `song-item ${bulkSelectionMode ? 'select-mode' : ''} ${isSelected ? 'bulk-selected' : ''}`;
        row.dataset.id = String(trackId);
        row.innerHTML = `
            <button class="song-select-marker ${isSelected ? 'selected' : ''}" onclick="event.stopPropagation(); window.toggleTrackSelection('${safeTrackId}', '${source}', window.tempNavidromeTracks && window.tempNavidromeTracks['${safeTrackId}'])" title="Select">
                <i data-lucide="${isSelected ? 'check' : 'circle'}"></i>
            </button>
...
function startHomeArtistPlayback(shuffleStart = false) {
    if (!isUserAuthenticated()) {
        showRegistrationRequiredOverlay('playback');
        return;
    }

    const tracks = Array.isArray(state.homeArtistSongs) ? state.homeArtistSongs.slice() : [];
    if (!tracks.length) {
        showToast(t('no_tracks_found', 'No tracks found'));
        return;
    }
...
window.playHomeArtist = () => {
    startHomeArtistPlayback(false);
};

window.shuffleHomeArtist = () => {
    startHomeArtistPlayback(true);
};

window.addHomeArtistToPlaylist = () => {
    const tracks = Array.isArray(state.homeArtistSongs) ? state.homeArtistSongs.slice() : [];
    if (!tracks.length) {
        showToast(t('no_tracks_found', 'No tracks found'));
        return;
...
window.openHomeArtist = async (artistName = '') => {
    ...
    homeArtistReturnTab = currentTab === 'artists' ? 'artists' : 'home';

    await ensureNavidromeSongsLoadedForArtistPage();
    let songs = mapArtistSongsFromCache(query);

    if (!songs.length && typeof window.searchNavidrome === 'function') {
        try {
            const searchResults = await window.searchNavidrome(query);
            songs = (searchResults || [])
                .map(normalizeHomeTrack)
                .filter((track) => track?.source === 'navidrome')
                .filter((track) => normalizeArtistKey(track.artist) === normalizeArtistKey(query));
        } catch (error) {
            console.warn('[ARTIST] Search fallback failed:', error);
        }
    }
    ...
    const resolvedName = songs[0]?.artist || query;
    const albums = mapArtistAlbums(resolvedName, songs);
    const cover = albums[0]?.cover || songs[0]?.cover || DEFAULT_COVER;
    homeArtistState = {
        name: resolvedName,
        cover,
        songs,
    ...
    renderHomeArtistTab();
    closeHomeContextMenu();
    window.switchTab('home-artist');
};

window.closeHomeArtistTab = () => {
    state.homeArtistSongs = [];
    const target = homeArtistReturnTab === 'artists' ? 'artists' : 'home';
    homeArtistReturnTab = 'home';
...
function buildAICandidateTracks(seedTracks = [], desiredLimit = 320, options = {}) {
    ...
    const seedAlbums = new Set((seedTracks || []).map((track) => String(track?.album || '').toLowerCase()).filter(Boolean));
    const seedArtistList = Array.from(seedArtists);
    const seedGenreList = Array.from(seedGenres);
    const seedAlbumList = Array.from(seedAlbums);
    const hasSeedContext = seedArtistList.length > 0 || seedGenreList.length > 0 || seedAlbumList.length > 0;

    const scored = sourceList.map((track) => {
        const artist = String(track.artist || '').toLowerCase();
        const genre = String(track.genre || '').toLowerCase();
        const album = String(track.album || '').toLowerCase();
        const artistMatch = seedArtistList.some((item) => item && (artist.includes(item) || (artist && item.includes(artist))));
        const genreMatch = seedGenreList.some((item) => item && (genre.includes(item) || (genre && item.includes(genre))));
        const albumMatch = seedAlbumList.some((item) => item && (album.includes(item) || (album && item.includes(album))));

        if (requireContextMatch && hasSeedContext && !artistMatch && !genreMatch && !albumMatch) {
            return null;
        }
        ...
        if (albumMatch) score += 16;
        if (artistMatch && albumMatch) score += 9;
        if (genreMatch && albumMatch) score += 5;
        if (!hasSeedContext) score += 1;
...
async function maybeExtendSmartShuffleQueue(force = false) {
    ...
    const playbackBaseView = state.currentIndex === -1
        ? getCurrentListView(playbackTab, false)
        : getPlaybackListView();
    const playlistScope = typeof playbackTab === 'number';
    ...
    const contextSeeds = playlistScope
        ? (Array.isArray(playbackBaseView) ? playbackBaseView : [])
            .slice(0, 280)
            .map((track) => ({
                navidrome_id: track?.navidromeId || track?.id || '',
                title: track?.title || '',
            ...
            .filter((track) => track.title || track.artist || track.album || track.genre)
        : [];
    const combinedSeeds = contextSeeds.length ? [...recentSeeds, ...contextSeeds] : recentSeeds;
    ...
    (Array.isArray(playbackBaseView) ? playbackBaseView : []).forEach((track) => {
        const id = String(track?.navidromeId || track?.id || '').trim();
        if (id) excludeIdsSet.add(id);
    });
    const excludeIds = Array.from(excludeIdsSet);

    const needed = Math.max(1, SMART_SHUFFLE_FILL_TARGET - tracksAhead);
    let candidateTracks = buildAICandidateTracks(combinedSeeds, 260, {
        excludeIds,
        requireContextMatch: playlistScope
    });
    if (!candidateTracks.length && playlistScope) {
        candidateTracks = buildAICandidateTracks(combinedSeeds, 260, {
            excludeIds,
...
window.toggleSmartShuffle = () => {
    if (!isUserAuthenticated()) {
        ...
        localStorage.setItem(SMART_SHUFFLE_STORAGE_KEY, 'false');
        updateSmartShuffleUI();
        showRegistrationRequiredOverlay('smart-shuffle');
        return;
    }
...
function updateHomeMiniPlayer() {
    ...
    const miniViews = [
        ...
        {
            player: document.getElementById('homeArtistMiniPlayer'),
            progress: document.getElementById('homeArtistMiniProgress'),
            cover: document.getElementById('homeArtistMiniCover'),
            title: document.getElementById('homeArtistMiniTitle'),
            artist: document.getElementById('homeArtistMiniArtist'),
            playIcon: document.getElementById('homeArtistMiniPlayIcon'),
            timeCur: document.getElementById('homeArtistMiniTimeCur'),
            timeDur: document.getElementById('homeArtistMiniTimeDur')
        },
        {
    ...
    miniViews.forEach((view) => {
        ...
        if (!currentTrack || !dom.audio || !dom.audio.src) {
            ...
            applyArtistLinkState(view.artist, '');
            return;
        }

        view.player.classList.add('visible');
        view.title.textContent = currentTrack.title || t('unknown_title', 'Unknown');
        const artistName = currentTrack.artist || t('unknown_artist', 'Unknown Artist');
        view.artist.textContent = artistName;
        applyArtistLinkState(view.artist, artistName);
...
window.openZenFromMiniPlayer = () => {
    if (!state.isZen) {
        ...
        const homeArtistSection = document.getElementById('homeArtistSection');
        ...
        zenMiniRestoreContext = {
            ...
            homeArtistScrollTop: homeArtistSection ? homeArtistSection.scrollTop : 0,
...
function updateMobileFullscreenPlayer() {
    ...
    applyArtistLinkState(artist, displayArtist);
...
function initHomeMiniVolumeControls() {
    const bars = [
        ...
        document.getElementById('homeArtistMiniVolBar'),
...
function updateHomeVisibility(force = false) {
    ...
    const isHomeArtistTab = state.currentTab === 'home-artist';
    ...
    const shouldShow = isHomeTab || isHomeAlbumTab || isHomeArtistTab || isHomePlaylistsTab || isAlbumsTab || isArtistsTab;
    ...
    const homeArtistSection = document.getElementById('homeArtistSection');
    if (homeArtistSection) homeArtistSection.style.display = isHomeArtistTab ? 'flex' : 'none';
    ...
    if (shouldShow || isPlaylistDetailTab) {
        ...
        if (isHomeArtistTab) renderHomeArtistTab();
...
function resetUI() {
    ...
    applyArtistLinkState(dom.artistName, '');
    dom.mainCover.src = "";
...
function renderSidebarQueue() {
    ...
    containers.forEach(container => {
        ...
        queue.forEach((track, idx) => {
            ...
            addToPlaylistBtn.className = 'mini-btn';
            addToPlaylistBtn.title = t('add_to_playlist', 'Add to playlist');
            addToPlaylistBtn.style.cssText = 'background: transparent; border: none; cursor: pointer; padding: 6px; color: var(--text-dim); transition: 0.2s;';
            addToPlaylistBtn.innerHTML = '<i data-lucide="plus" style="width:18px; height:18px;"></i>';
            addToPlaylistBtn.onclick = (e) => {
                e.stopPropagation();
                window.tempPendingTrack = track;
                window.openPlaylistPickerMulti(trackId, source);
            };
            addToPlaylistBtn.onmouseover = () => addToPlaylistBtn.style.color = 'var(--accent)';
            addToPlaylistBtn.onmouseout = () => addToPlaylistBtn.style.color = 'var(--text-dim)';

            const favBtn = document.createElement('button');
            favBtn.className = 'mini-btn';
            favBtn.title = t('favorite', 'Favorite');
            favBtn.style.cssText = 'background: transparent; border: none; cursor: pointer; padding: 6px; color: var(--text-dim); transition: 0.2s;';
            favBtn.innerHTML = `<i data-lucide="heart" style="width:18px; height:18px; fill: ${track.isFavorite ? 'var(--accent)' : 'none'}; color: ${track.isFavorite ? 'var(--accent)' : 'currentColor'};"></i>`;
            favBtn.onclick = (e) => {
                e.stopPropagation();
                Promise.resolve(window.toggleFav(trackId, source))
                    .catch(() => {})
                    .finally(() => {
                        renderSidebarQueue();
                        renderMiniQueueModal();
            ...
            favBtn.onmouseover = () => favBtn.style.color = 'var(--accent)';
            favBtn.onmouseout = () => favBtn.style.color = 'var(--text-dim)';
            ...
            const moveUpBlockedByCurrent = currentQueueIdx !== -1 && (idx === currentQueueIdx || (idx - 1) === currentQueueIdx);
            moveUpBtn.disabled = idx === 0 || moveUpBlockedByCurrent;
            ...
            const moveDownBlockedByCurrent = currentQueueIdx !== -1 && (idx === currentQueueIdx || (idx + 1) === currentQueueIdx);
            moveDownBtn.disabled = idx === queue.length - 1 || moveDownBlockedByCurrent;
            ...
            div.appendChild(addToPlaylistBtn);
...
function bindQueueDragAndDrop(row, viewIndex, onDropDone = null) {
    ...
    row.addEventListener('dragover', (event) => {
        ...
        row.style.borderTop = placeAfter ? '' : '1px dashed rgba(var(--accent-rgb),0.85)';
        row.style.borderBottom = placeAfter ? '1px dashed rgba(var(--accent-rgb),0.85)' : '';
    });
    ...
    row.addEventListener('drop', async (event) => {
        ...
        const rect = row.getBoundingClientRect();
        const placeAfter = event.clientY > (rect.top + rect.height / 2);
        const toIndex = Number(viewIndex) + (placeAfter ? 1 : 0);
...
function applyReorderedSubsetToBaseView(baseView, originalSubset, reorderedSubset) {
    if (!Array.isArray(baseView) || !Array.isArray(originalSubset) || !Array.isArray(reorderedSubset)) return null;
    if (!baseView.length || !originalSubset.length || originalSubset.length !== reorderedSubset.length) return null;

    const remainingById = new Map();
    originalSubset.forEach((track) => {
        const key = getTrackPlaybackId(track);
        if (!key) return;
        remainingById.set(key, (remainingById.get(key) || 0) + 1);
    });

    const targetIndices = [];
    baseView.forEach((track, index) => {
        const key = getTrackPlaybackId(track);
        if (!key) return;
        const remaining = Number(remainingById.get(key) || 0);
        if (remaining <= 0) return;
        targetIndices.push(index);
        remainingById.set(key, remaining - 1);
    });

    if (targetIndices.length !== reorderedSubset.length) return null;

    const nextBaseView = [...baseView];
    targetIndices.forEach((baseIndex, i) => {
        nextBaseView[baseIndex] = reorderedSubset[i];
    });
    return nextBaseView;
}

function getPlaylistTrackOrderKey(track) {
    if (!track) return '';
    if (track.source === 'navidrome') {
        const navId = normalizeQueueId(track.navidromeId || track.id || track.url);
        return navId ? `navidrome:${navId}` : '';
    }
    const localId = Number(track.id);
    return Number.isFinite(localId) ? `local:${localId}` : '';
}

async function persistReorderedPlaybackBaseView(nextBaseView) {
    const playbackTab = state.currentIndex === -1
        ? state.currentTab
        : (state.playbackTab ?? state.currentTab);
    if (!Array.isArray(nextBaseView) || !nextBaseView.length) return false;

    if (playbackTab === 'all') {
        state.library = [...nextBaseView];
        return true;
    }

    if (playbackTab === 'fav') {
        const reorderedFav = [...nextBaseView];
        let pointer = 0;
    ...
    if (playbackTab === 'home-album') {
        state.homeAlbumSongs = [...nextBaseView];
        return true;
    }
    if (playbackTab === 'home-artist') {
        state.homeArtistSongs = [...nextBaseView];
        return true;
    }
    ...
    if (typeof playbackTab === 'number') {
        ...
        const navidromeSongIds = navidromeSongs
            .map((track) => String(track.navidromeId || track.id || '').trim())
            .filter(Boolean);
        const trackOrder = moved
            .map((track) => getPlaylistTrackOrderKey(track))
            .filter(Boolean);

        playlist.songIds = songIds;
        playlist.navidromeSongs = navidromeSongs;
        playlist.navidromeSongIds = navidromeSongIds;
        playlist.trackOrder = trackOrder;
...
window.moveQueueTrackToIndex = async (fromViewIndex, toViewIndex, options = {}) => {
    const fromIndex = Number(fromViewIndex);
    const rawToIndex = Number(toViewIndex);
    const insertMode = !!options?.insertMode;
    if (!Number.isFinite(fromIndex) || !Number.isFinite(rawToIndex)) return;
    ...
    if (insertMode) {
        toIndex = Math.max(0, Math.min(queue.length, toIndex));
    } else {
    ...
    if (insertMode) {
        if (toIndex > fromIndex) effectiveToIndex = toIndex - 1;
        effectiveToIndex = Math.max(0, Math.min(queue.length - 1, effectiveToIndex));
    }
    if (fromIndex === effectiveToIndex) return;
    ...
    const currentQueueIdx = currentTrack
        ...
        : -1;
    if (currentQueueIdx !== -1 && (fromIndex === currentQueueIdx || effectiveToIndex === currentQueueIdx)) {
        showToast(t('queue_reorder_playing_locked', 'Cannot move the currently playing track'));
        return;
    }

    const fromTrack = queue[fromIndex];
    const toTrack = queue[effectiveToIndex];
    ...
    if (fromInBase) {
        const baseQueueTracks = queue.filter((track) => inBase(track));
        const fromBaseQueueIndex = findTrackIndexInViewById(baseQueueTracks, getTrackPlaybackId(fromTrack));
        const toBaseQueueIndex = findTrackIndexInViewById(baseQueueTracks, getTrackPlaybackId(toTrack));
        if (fromBaseQueueIndex < 0 || toBaseQueueIndex < 0) return;
        const reorderedBaseQueue = moveArrayItem(baseQueueTracks, fromBaseQueueIndex, toBaseQueueIndex);
        const nextBaseView = applyReorderedSubsetToBaseView(baseView, baseQueueTracks, reorderedBaseQueue);
        if (!nextBaseView) return;
        const changed = await persistReorderedPlaybackBaseView(nextBaseView);
        if (!changed) return;
    } else {
    ...
    else if (state.currentTab === 'home-artist') renderHomeArtistTab();
    else if (state.currentTab === 'home' || state.currentTab === 'home-artist' || state.currentTab === 'home-playlists') updateHomeVisibility();
    else renderLibrary();
    renderSidebarQueue();
    renderMiniQueueModal();
};
...
window.playTrack = (id) => {
    ...
    applyArtistLinkState(dom.artistName, displayArtist);
...
function updateMobileBottomNavState(tab) {
    ...
    if (tab === 'home' || tab === 'home-album' || tab === 'home-artist') document.getElementById(map.home)?.classList.add('active');
...
window.switchTab = (tab) => {
    ...
    rememberLastNonInfoTab(previousTab);
    ...
    const persistedTab = (tab === 'home-album' || tab === 'home-artist' || tab === 'home-playlists') ? 'home' : tab;
    ...
    if (tab === 'home' || tab === 'home-album' || tab === 'home-artist' || tab === 'home-playlists' || tab === 'albums' || tab === 'artists') {
        if (tab === 'home' || tab === 'home-album' || tab === 'home-artist' || tab === 'home-playlists') {
        ...
        if (homeArtistSection) homeArtistSection.style.display = 'none';
        ...
        try {
            ...
            btn.onclick = closeInfoTabToLastContext;
...
window.bulkDeleteSelectedTracks = async () => {
    ...
    if (state.currentTab === 'home-album' || state.currentTab === 'home-artist') {
        showToast(t('delete_not_available_album', 'Delete is unavailable in album view'));
        window.disableBulkSelection(true);
        if (state.currentTab === 'home-album') renderHomeAlbumTab();
        else renderHomeArtistTab();
        return;
    }
...
function initAudioEvents() {
    dom.audio.onplay = () => { 
        ...
        updatePlayIcon(true);
        updateHomeMiniPlayer();
        if (state.currentTab === 'home' || state.currentTab === 'home-album' || state.currentTab === 'home-artist' || state.currentTab === 'home-playlists') updateHomeVisibility();
        if ('mediaSession' in navigator) {
            navigator.mediaSession.playbackState = 'playing';
        }
    };
...
function updateVolumeUI() {
    ...
    const miniFills = [
        ...
        document.getElementById('homeArtistMiniVolFill'),
...
window.toggleZen = (enable) => {
    ...
    if (enable) {
        ...
        if (zenOpenedFromMiniPlayer) {
            ...
            if (homeArtistSection && zenMiniRestoreContext?.tab === 'home-artist') {
                homeArtistSection.scrollTop = Number(zenMiniRestoreContext.homeArtistScrollTop || 0);
            }
...
export async function initApp() {
    ...
    // Open Home immediately on app start (desktop + mobile), without waiting for background loads.
    ...
    (async () => {
        ...
        if (state.currentTab === 'home' || state.currentTab === 'home-album' || state.currentTab === 'home-artist' || state.currentTab === 'home-playlists') {
            updateHomeVisibility(true);
        }
        if (state.smartShuffleEnabled) {
            setTimeout(() => {
